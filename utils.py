...
valid_num_dic = {'Amazon_clothing': 17, 'Amazon_eletronics': 36, 'dblp': 27, 'Cora': 2}
...
def load_data(dataset_source):
    ...
    OOD_class_list = []
    for i in range(10):
        OOD_class = class_list_test[-1]
        class_list_test.pop()
        OOD_class_list.append(OOD_class)
    return adj, features, labels, degree, class_list_train, class_list_valid, class_list_test, OOD_class_list, id_by_class
    # Without OOD Mode
...
def task_generator(id_by_class, class_list, n_way, k_shot, m_shot):
    class_selected = random.sample(class_list, n_way)
    id_support = []
    id_query = []
    for cla in class_selected:
        temp = random.sample(id_by_class[cla], k_shot + m_shot)
        id_support.extend(temp[:k_shot])
        id_query.extend(temp[k_shot:])
    return np.array(id_support), np.array(id_query), class_selected

def OOD_task_generator(id_by_class, class_list, OOD_sample_list, n_way, k_shot, m_shot):
    ...
    for cla in ID_class_selected:
        ...
        id_support.extend(temp[:k_shot])
        id_query.extend(temp[k_shot:])
    OOD_sample = random.sample(OOD_sample_list, m_shot)
    return np.array(id_support), np.array(id_query), np.array(OOD_sample), ID_class_selected
